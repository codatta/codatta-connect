...
// Email login widget component that handles email verification flow
export default function EmailLoginWidget(props: {
  ...
  const { email } = props;
  // State management for countdown timer, loading states and error messages
  ...
  // Start a 60-second countdown timer for code resend
  ...
  // Send verification code when email is provided
  useEffect(() => {
    if (!email) return;
    sendEmailCode(email);
  }, [email]);

  // Handle OTP input changes and verify code
    ...
    <TransitionEffect>
      ...
      <div className="xc-flex xc-flex-col xc-items-center xc-justify-center xc-mb-12">
        ...
        <div className="xc-flex xc-flex-col xc-items-center xc-justify-center xc-mb-8 xc-h-16">
          {getCodeError ? (
            <div className="xc-text-[#ff0000] xc-text-center">
              <p className="xc-px-8">{getCodeError}</p>
            </div>
          ) : sendingCode ? (
            <Loader2 className="xc-animate-spin"></Loader2>
          ) : (
            <>
              <p className="xc-text-lg xc-mb-1">
        ...
        <div className="xc-mb-2 xc-h-12">
          <Spin spinning={loading} className="xc-rounded-xl">
            <InputOTP
              ...
              disabled={loading}
              className="disabled:xc-opacity-20"
            >
              <InputOTPGroup>
                <div
                  className={cn(
                    "xc-flex xc-gap-2",
                    loading ? "xc-opacity-20" : ""
                  )}
                >
      ...
      <div className="xc-text-center xc-text-sm xc-text-gray-400">
        ...
        {count ? (
          ...
          <button onClick={() => sendEmailCode(email)}>Send again</button>
        )}
      </div>
    </TransitionEffect>
  );
